...
This is deliberately framed as an investigation, not a demonstration. A project that asserts "my agent doesn't hallucinate" is weak. A project that measures three architectures against a held-out set and reports exactly which error classes each one eliminates — and which it doesn't — is defensible under questioning.
...
MASE scales forecast errors by the **in-sample (training) MAE of a naive method** — not by the baseline's performance on the test period.
...
**Worked example.** Suppose across the training data the average one-step seasonal-naive absolute error is **20 units**. Call this the scale factor *S*; it is computed once and is a property of the series.
...
**The tell that distinguishes it from a ratio-of-two-models.** In Hyndman's own quarterly beer example, the seasonal naive method scores MASE = **0.94**, not 1.00. If MASE were model-MAE ÷ baseline-MAE on the test set, the baseline would score exactly 1.00 by construction. It does not.
...
Handling: exclude affected series from MASE aggregation, **report the excluded count explicitly**, and fall back to RelMAE for those series. Silently dropping them is the kind of thing an interviewer finds.
...
| DimensionGuards against                              |                                      |
| ---------------------------------------------------- | ------------------------------------ |
| **Answer rate** (n attempted / n answerable)         | Over-refusal                         |
| **Answer accuracy** (of attempted)                   | Reckless answering                   |
| **Claim precision** (validated claims / claims made) | Fabrication, binding errors          |
| **Claim recall** (required claims made / required)   | Thin, incomplete answers             |
| **False refusal rate** (on answerable)               | Over-refusal                         |
| **Correct abstention rate** (on unanswerable)        | Over-confidence                      |
| **Latency, tokens, cost per query**                  | Reliability bought at absurd expense |

Every figure reported as `k / n`, not a bare percentage. "99.2%" is meaningless without *n* and without the definition of success.
...
| Arm A: free-formArm B: structuredArm C: validated |     |     |                       |
| ------------------------------------------------- | --- | --- | --------------------- |
| Answer rate                                       | k/n | k/n | k/n                   |
| Answer accuracy                                   | k/n | k/n | k/n                   |
| Claim precision                                   | k/n | k/n | k/n                   |
| Claim recall                                      | k/n | k/n | k/n                   |
| Value fabrication                                 | k   | k   | **0 by construction** |
| Binding errors                                    | k   | k   | **0 by construction** |
| Directional errors                                | k   | k   | k                     |
| Incomplete comparison                             | k   | k   | k                     |
| False refusal                                     | k/n | k/n | k/n                   |
| Median latency                                    | ms  | ms  | ms                    |
| Cost / query                                      | $   | $   | $                     |

*Held-out set, n = \_\_. Forecast layer: MASE \_\_ (excl. \_\_ undefined series), RelMAE vs seasonal naive \_\_.*

**And a defensible ninety-second story:**

> Analytical LLM systems fail by stating wrong numbers confidently, and people act on them. I wanted to know how much of that is structurally preventable rather than just prompt-tunable. So I built the same analytical agent three ways — free narration, structured claims with source references, and validated claims where deterministic code renders the values — and measured all three on a held-out question set. Value fabrication and binding errors go to zero in the third architecture, by construction. Directional and interpretation errors do not. Here's what the constraint costs in latency and answer usefulness, and here's the taxonomy of what still gets through.

---

## 11. The guarantee, stated narrowly

**Arm C guarantees:** every numeric value displayed is read by deterministic code directly from a source row whose entity, metric, period, and model version were validated against the claim.

**Arm C does not guarantee:** that the right row was selected, that the comparison set was complete, that the direction of change is described correctly, that the interpretation is sound, or that a material fact was not omitted.
